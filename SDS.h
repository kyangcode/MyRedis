...
namespace std
{
	class SDS
	{
		friend ostream &operator<<(ostream &out, const SDS &sds);
	public:
		SDS();
		SDS(const string &s);
		SDS(const SDS &sds);
		...
		size_t size() const; // 返回sds长度
		size_t avail() const; // 返回sds未使用空间字节数
		
		void clear(); // 清空sds

		SDS &cat(const string &s); // 将c字符串拼接到sds后面
		...
		SDS &trim(const string &s); // sds修剪两边在str出现过的字符 
	...
	ostream &operator<<(ostream &out, const SDS &sds); // 打印SDS
}
